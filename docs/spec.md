...
* 画像入力（マルチモーダル）：
  ...
  * 画像URLを LLM に渡すのは条件付き。
  * 条件：

    * 本文が CJK（日本語/中国語/韓国語）に該当 → 1000 文字以下
    * それ以外（主に英語想定） → 500 words 以下
    * かつ 画像が 3 枚以上
  * 画像は HTML 中の `<img src>` を抽出し、トラッキング/ピクセル系を除外しつつ最大5件に制限。
  * 見出し画像（OG/Twitter card）を別途抽出し、メール本文に表示する（LLM入力とは独立）。
...
│   ├── summarizer.py                # OpenAI 要約ロジック（条件付き画像入力）
...
* 本文抽出時、CJKは1000文字以下、非CJKは500 words以下、かつ画像が3枚以上の場合は、画像 URL も LLM に渡してマルチモーダル要約する（HTML内の出現順で添付、最大5件）。それ以外はテキストのみ。
